...
class W2VSubs(Metaphor):
    ...
    def metaphorize(self, text=None, num_neighbors=5, fast_desired=False):
        words_tagged = self.deconstruct(text, PoS={'NOUN', 'ADJ', 'ADV'})
        words_list = [w for w, tag in words_tagged]
        closest_n = self.create(words_list, num_neighbors=num_neighbors, fast_desired=fast_desired)
        return self.reconstruct(text, closest_n)
